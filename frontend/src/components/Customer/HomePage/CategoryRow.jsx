...
const CategoryRow = ({
  ...
  const { toggleWishlist, isInWishlist } = useWishlistStore(); // Destructure
  ...
  return (
    <div id={id} className="mb-16 scroll-mt-24">
      ...
      >
        {items.map((asset) => {
          const isWishlisted = isInWishlist(asset._id);
          return (
            <div
              key={asset._id}
            ...
            >
              <div className="aspect-[4/3] w-full overflow-hidden rounded-t-2xl bg-gray-100 relative">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleWishlist(asset._id);
                  }}
                  className="absolute top-3 right-3 p-2 bg-white/80 backdrop-blur rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white text-rose-500"
                >
                  <Heart
